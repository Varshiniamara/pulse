...
export default function ConversationList({ conversations, activeId, onSelect, searchQuery }: ConversationListProps) {
  ...
  const getChatName = (chat: any) => {
    ...
    const otherUser = chat.participants?.find((p: any) => String(p?._id || p?.id || '') !== currentUserId);
  ...
  if (filteredDMs.length === 0 && filteredGroups.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-violet-400 text-sm">
  ...
  return (
    <div className="flex flex-col gap-1 overflow-y-auto flex-1 px-2 pb-4 scrollbar-thin">
      {filteredDMs.length > 0 && (
        <div>
          <p className="text-[10px] font-extrabold text-violet-400/60 uppercase tracking-[0.2em] px-3 py-2 mt-1">
            Direct Messages
          </p>
      ...
      {filteredGroups.length > 0 && (
        <div>
          <p className="text-[10px] font-extrabold text-violet-400/60 uppercase tracking-[0.2em] px-3 py-2 mt-3">
            Groups
          </p>
          <div className="flex flex-col gap-0.5">
